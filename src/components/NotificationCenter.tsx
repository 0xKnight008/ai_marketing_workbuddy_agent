...
function RuleEditor({ flow, rule, accounts, apiBase, onSaved, onError }: {
  ...
  return <article className="rounded-xl border border-ink/15 p-4">
    <div className="flex flex-wrap items-center gap-2">
      ...
      {rule
        ? <span className="rounded bg-meadow/20 px-2 py-0.5 text-xs">on · {rule.channel}{rule.kind === 'weekly_report' ? ` · ${rule.weeklyDeliveryMode}` : ''}</span>
        : <span className="rounded bg-ink/10 px-2 py-0.5 text-xs">off</span>}
      <button type="button" onClick={() => setOpen(value => !value)} className="ml-auto rounded border px-3 py-1 text-xs">{open ? 'Close' : rule ? 'Edit' : 'Enable'}</button>
    </div>
    <p className="mt-1 text-xs text-ink-soft">Runs {flow.schedule}. {flow.description}</p>
    {open && <div className="mt-3 space-y-2 rounded-lg bg-paper p-3">
      <div className="flex flex-wrap gap-2">
        {(['email', 'discord'] as const).map(option => <button key={option} type="button" onClick={() => setChannel(option)} aria-pressed={channel === option} className={`rounded border px-3 py-1 text-sm ${channel === option ? 'bg-sky-deep text-white' : ''}`}>{option}</button>)}
      </div>
      {channel === 'email' && <input type="email" value={email} onChange={event => setEmail(event.target.value)} placeholder="Recipient email (blank = workspace owner)" className="w-full rounded border border-ink/20 bg-paper-card px-3 py-2 text-sm" />}
      {channel === 'discord' && (discordAccounts.length === 0
        ? <p className="text-xs text-ink-soft">No connected Discord account with publish capability — connect one under Accounts first.</p>
      ...
      {flow.kind === 'weekly_report' && <div className="flex flex-wrap gap-2">
        <select value={weeklyTemplate} onChange={event => setWeeklyTemplate(event.target.value)} className="rounded border border-ink/20 bg-paper-card px-3 py-2 text-sm">
          <option value="content_recap">Content recap</option><option value="comment_insights">Comment insights</option><option value="product_opportunities">Product opportunities</option><option value="review_attribution">Review attribution</option><option value="community_digest">Community digest</option>
        </select>
        <select value={weeklyMode} onChange={event => setWeeklyMode(event.target.value as 'approval' | 'auto')} className="rounded border border-ink/20 bg-paper-card px-3 py-2 text-sm">
          <option value="approval">approval — I release each digest</option><option value="auto">auto — deliver directly</option>
        </select>
      </div>}
      <div className="flex gap-2">
        <button type="button" disabled={busy || (channel === 'discord' && !accountId)} onClick={() => void save()} className="rounded bg-sky-deep px-4 py-2 text-sm font-bold text-white disabled:opacity-50">{busy ? 'Saving…' : 'Save rule'}</button>
        {rule && <button type="button" disabled={busy} onClick={() => void disable()} className="rounded border px-4 py-2 text-sm">Disable</button>}
      </div>
      <p className="text-xs text-ink-soft">Saving this rule is your standing approval for this recurring delivery. Report content is platform-validated; suggested outbound actions inside reports still need per-action approval.</p>
    </div>}
  </article>;
}
...
export function NotificationCenter({ apiBase: base, accounts }: { apiBase: string; accounts: ConnectedAccount[] }) {
  ...
  return <section aria-label="Scheduled delivery" className="sketch bg-paper-card p-6 shadow-paint-sm">
    <h2 className="font-display text-3xl">Scheduled delivery</h2>
    <p className="mt-2 text-sm text-ink-soft">The daily operations loop, delivered: morning task push, evening recap, weekly report generation with approval delivery, and urgent risk alerts. Times are UTC. Enabling a flow is your standing approval for that recurring delivery; AI-suggested outbound actions stay per-action approval-gated.</p>
    <button type="button" disabled={busy} onClick={load} className="mt-3 rounded border px-4 py-2 text-sm">{busy ? 'Loading…' : loaded ? 'Refresh scheduled delivery' : 'Load scheduled delivery'}</button>
    ...
    {loaded && <div className="mt-4 space-y-4">
      ...
      <article className="rounded-xl border border-ink/15 p-4">
        <h3 className="text-lg font-semibold">Delivery events</h3>
        {events.length === 0 && <p className="mt-2 text-sm text-ink-soft">No events yet. They appear as scheduled flows run.</p>}
        <ul className="mt-2 space-y-2">{events.map(event => <li key={event.id} className="rounded border border-ink/10 p-3">
          ...
          {event.error && <p className="mt-1 text-xs text-sunset-deep">Error: {event.error}</p>}
          <div className="mt-2 flex gap-2">
            {event.kind === 'weekly_report' && event.status === 'pending_approval' && <button type="button" onClick={() => void act(event.id, 'approve')} className="rounded bg-sky-deep px-3 py-1 text-xs font-bold text-white">Approve delivery</button>}
            {event.kind === 'urgent_risk' && event.status === 'sent' && <button type="button" onClick={() => void act(event.id, 'acknowledge')} className="rounded border px-3 py-1 text-xs">Acknowledge</button>}
            {event.kind === 'urgent_risk' && (event.status === 'sent' || event.status === 'acknowledged') && <button type="button" onClick={() => void act(event.id, 'resolve')} className="rounded border px-3 py-1 text-xs">Resolve</button>}
          </div>
        </li>)}</ul>
      </article>
    </div>}

...
type Section = 'dashboard' | 'pipelines' | 'imports' | 'insights' | 'topics' | 'notifications' | 'accounts' | 'activity' | 'settings';
...
function InsightReportBody({ report }: { report: InsightReportView }) {
  ...
  return <div className="space-y-6 p-6 md:p-8">
    ...
    {report.template === 'content_recap' && (() => { const recap = body as unknown as RecapReport; return <>
      ...
      {recap.nextTopics?.length > 0 && <section><h3 className="text-lg font-semibold">Next batch: {recap.nextTopics.length} topic ideas</h3><ol className="mt-3 list-decimal space-y-1 pl-6 text-sm">{recap.nextTopics.map((topic, index) => <li key={index}>{topic}</li>)}</ol></section>}
      {Boolean(recap.draftScripts?.length) && <section><h3 className="text-lg font-semibold">Script drafts · review before publishing</h3><div className="mt-3 space-y-3">{recap.draftScripts!.map((script, index) => <article key={index} className="rounded-xl border border-ink/15 bg-paper-card p-4"><h4 className="font-semibold">{script.title}</h4><p className="mt-2 whitespace-pre-wrap text-sm">{script.body}</p><CitationList citations={script.citations} /></article>)}</div></section>}
    ...
    {report.template === 'comment_insights' && (() => { const insights = body as unknown as CommentReport; return <>
      {insights.demandRanking?.length > 0 && <section><h3 className="text-lg font-semibold">Fan demand ranking</h3><div className="mt-3 space-y-2">{insights.demandRanking.map((demand, index) => <div key={index} className="flex items-start gap-3 rounded-lg bg-paper-card p-3"><span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-sky-deep text-xs text-white">{index + 1}</span><div className="flex-1"><p className="text-sm font-medium">{demand.demand} <span className="text-xs text-ink-soft">· {demand.citations ? new Set(demand.citations.map(c => c.ref)).size : 0} cited sources</span></p><CitationList citations={demand.citations} /></div></div>)}</div></section>}
      {insights.frequentQuestions?.length > 0 && <section><h3 className="text-lg font-semibold">Frequent questions</h3><div className="mt-3 space-y-2">{insights.frequentQuestions.map((question, index) => <div key={index} className="rounded-lg bg-paper-card p-3"><p className="text-sm font-medium">{question.question} <span className="text-xs text-ink-soft">· {question.citations ? new Set(question.citations.map(c => c.ref)).size : 0} cited sources</span></p><CitationList citations={question.citations} /></div>)}</div></section>}
    ...
    {report.template === 'review_attribution' && (() => { const review = body as unknown as ReviewReport; return <>
      {Boolean(body._reviewRanking) && <p className="rounded-xl bg-sky-pale p-3 text-sm">Top {review.issueClusters.length} issues (up to 5): ordered by model-assessed severity, then distinct cited reviews. This is not a sales-impact or full-dataset frequency ranking. SKU labels are restricted to the cited source metadata.</p>}
      {review.issueClusters?.length > 0 && <section><h3 className="text-lg font-semibold">Complaint themes</h3><div className="mt-3 space-y-3">{review.issueClusters.map((cluster, index) => <div key={index} className="rounded-xl border border-ink/15 bg-paper-card p-4"><div className="flex flex-wrap items-center gap-2"><p className="text-sm font-semibold">{cluster.theme}</p><PriorityChip value={cluster.severity} /><span className="text-[11px] text-ink-soft">{cluster.citations ? new Set(cluster.citations.map(c => c.ref)).size : 0} cited reviews</span></div>{cluster.affectedSkus?.length > 0 && <div className="mt-2 flex flex-wrap gap-1.5">{cluster.affectedSkus.map((sku) => <span key={sku} className="rounded-full bg-sky-pale px-2 py-0.5 text-[11px]">SKU: {sku}</span>)}</div>}<CitationList citations={cluster.citations} /></div>)}</div></section>}
